...
def Adaboost(data,labelList,n_stump):
	...
	for i in range(n_stump):
		...
		# w_next=np.multiply(w,exp(-alpha*np.multiply(labelList,pre)))
		...
		print((predict==labelList).reshape(m).tolist()[0].count(True)/len(labelList))#打印正确率
